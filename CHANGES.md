...
- Multi-perspective CAA corroboration: `CaaChecker` (RFC 8659 tree-climbing, RFC 8657 `accounturi`/`validationmethods`, fail-closed on lookup errors) and `CaaCorroborator` run CAA checks across the configured MPIC handler — self-hosted agents (which now serve the `caa` check type) or Open MPIC's `check_type: caa`. Relevant when acme2certifier itself is the issuing CA. See [docs/mpic.md](docs/mpic.md) section 10.
- CAA check at order finalize: the new `[Order] mpic_caa_check` option (default `False`) corroborates CAA for every DNS identifier immediately before enrollment and refuses issuance with `urn:ietf:params:acme:error:caa`, marking the order invalid. Fails closed. Adds the `caa` ACME error type.
